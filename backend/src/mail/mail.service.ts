...
export class MailService {
  ...
  async sendRegistrationConfirmationEmail(to: string, firstName: string) {
    const appName = this.configService.get<string>('APP_NAME') ?? 'Terenick';
    const logo = this.buildLogoOptions();
    const loginUrl = this.buildFrontendAssetUrl('/login');
    const escapedFirstName = this.escapeHtml(firstName);
    ...
    return this.sendMail({
      ...
      from:
        this.configService.get<string>('EMAIL_FROM') ??
        this.configService.get<string>('EMAIL_USER'),
      subject: `${appName} - Votre compte a bien été créé`,
      attachments: logo.attachments,
      text: [
        `Bonjour ${firstName},`,
        '',
        `Votre compte ${appName} a bien été créé.`,
        'Vous pouvez maintenant vous connecter et utiliser votre espace.',
        `Connexion: ${loginUrl}`,
        '',
        "Si vous n'etes pas a l'origine de cette inscription, contactez-nous rapidement.",
        '',
        "L'equipe Terenick",
      ].join('\n'),
      html: `
              ...
              <td align="center" style="padding:32px 16px;">
                <table role="presentation" width="100%" cellspacing="0" cellpadding="0" style="max-width:600px;border-collapse:collapse;background:#ffffff;border-radius:16px;overflow:hidden;box-shadow:0 12px 32px rgba(31,42,55,0.12);">
                  <tr>
                    <td style="padding:28px 32px;background:#3f5368;color:#ffffff;">
                      <img src="${escapedLogoSrc}" width="56" height="56" alt="${escapedAppName}" style="display:block;width:56px;height:56px;margin:0 0 18px;border-radius:12px;">
                      <p style="margin:0 0 8px;font-size:13px;letter-spacing:1px;text-transform:uppercase;color:#dbe8f4;">Bienvenue</p>
                      <h1 style="margin:0;font-size:28px;line-height:1.2;font-weight:700;">Votre compte a bien été créé</h1>
                    </td>
                  </tr>
                  <tr>
                    <td style="padding:32px;">
                      <p style="margin:0 0 16px;font-size:16px;line-height:1.6;">Bonjour ${escapedFirstName},</p>
                      <p style="margin:0 0 22px;font-size:16px;line-height:1.6;">
                        Votre compte ${escapedAppName} a bien été créé. Vous pouvez maintenant vous connecter et utiliser votre espace.
                      </p>
                      <p style="margin:0 0 26px;">
                        <a href="${escapedLoginUrl}" style="display:inline-block;padding:14px 22px;background:#3f5368;color:#ffffff;text-decoration:none;border-radius:8px;font-weight:700;">
                          Se connecter
                        </a>
                      </p>
                      <p style="margin:0 0 18px;font-size:14px;line-height:1.6;color:#66717d;">
